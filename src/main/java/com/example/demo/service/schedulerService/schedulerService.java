    ...
    public void createSQLCopy() {
        ...
        try {
            ...
            try(FileWriter writer = new FileWriter(banks, true);){
                ...
                for (Manufacture item : manufactures) {
            ...
            try(FileWriter writer = new FileWriter(cards, true);){
                ...
                for (Worker item : workers) {
                    writer.write("id "+item.getId()+ " firstName " + item.getFirstName() + " lastName " + item.getLastName() +  " middleName " + item.getMiddleName() +" manufactures id " + item.getManufacture().getId() + "\n");
                }
                writer.flush();
            }
            catch (IOException e)
